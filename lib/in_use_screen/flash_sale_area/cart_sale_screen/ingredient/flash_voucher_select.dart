...
import '../../../page/voucher_page/ingredient/voucher_select/item_voucher_select.dart';

class flash_voucher_select extends StatefulWidget {
  final Voucher voucher;
  final double cost;
  final VoidCallback ontap;
  const flash_voucher_select({super.key, required this.voucher, required this.cost, required this.ontap});

  @override
  State<flash_voucher_select> createState() => _general_selectState();
}

class _general_selectState extends State<flash_voucher_select> {
  final List<Voucher> voucherList = [];
  int count = 0;

  void getVoucherData() {
    final reference = FirebaseDatabase.instance.ref();
    reference.child("Flashsale").child('voucher').onValue.listen((event) {
      voucherList.clear();
